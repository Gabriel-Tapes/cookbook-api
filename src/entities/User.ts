...
export class User {
  ...
  set password(password: string) {
    if (!password)
      throw new Error(
        'User password error, cannot set a empty password for user'
      )
  ...
  constructor(
    { name, email, password, accessLevel }: Omit<UserProps, 'id'>,
    id?: string
  ) {
    if (!(name && email && password))
      throw new Error(
        'User props error, cannot create a user with name, email or password empty'
      )
